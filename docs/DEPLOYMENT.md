...
`docker-compose.prod.yml` runs with `APP_ENV=production`, `NODE_ENV=production`, `RUN_DB_SEED=0` by default, no host PostgreSQL port mapping, API-required frontend build args, and required Compose substitutions for `POSTGRES_PASSWORD`, `JWT_SECRET`, and `WEB_ORIGIN`. The production env file should be delivered through the deployment secret store; `.dockerignore` excludes `.env.*` files from Docker build context while preserving the checked-in example templates.
...
This validation rejects blank secrets, template domains, local origins, relative local-volume storage, unsafe backup directories, incomplete S3-compatible object storage settings, weak body-size settings, demo frontend fallback, and unapproved production seeding before the container stack starts. It is intentionally stricter than `.env.production.example`: the template uses blank secrets and `https://oa.example.com` as placeholders, while the real env file must use a deployment-managed PostgreSQL password, JWT secret, approved HTTPS origin, explicit absolute off-app `BACKUP_DIR`, and either absolute backed `FILE_STORAGE_DIR` plus `FILE_BACKUP_DIR` with `FILE_STORAGE_DRIVER=local` or complete `OBJECT_STORAGE_*` values with `FILE_STORAGE_DRIVER=s3`. Always run before `docker compose -f docker-compose.prod.yml up`.
