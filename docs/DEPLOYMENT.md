...
```bash
npm run configure:backend-server -- --ensure-github-environment --inspect-github-secrets --json
npm run configure:backend-server -- --storage-driver s3 --ensure-github-environment --inspect-github-secrets --json
```
...
Cloudflare Tunnel public-hostname configuration is also scripted. The command is dry-run by default; it reads the existing remotely-managed Tunnel configuration when credentials are present, preserves unrelated hostnames, upserts the `API_ORIGIN` hostname to `http://api:8787`, and makes the final rule `http_status:404`:

```bash
CLOUDFLARE_ACCOUNT_ID=<account-id> \
CLOUDFLARE_API_TOKEN=<api-token-with-cloudflare-tunnel-write> \
npm run configure:cloudflare-tunnel -- --env .env.production --tunnel <tunnel-uuid> --json

# Apply only after the redacted dry-run JSON is reviewed.
CLOUDFLARE_ACCOUNT_ID=<account-id> \
CLOUDFLARE_API_TOKEN=<api-token-with-cloudflare-tunnel-write> \
npm run configure:cloudflare-tunnel -- --env .env.production --tunnel <tunnel-uuid> --apply --json
```

The script uses Cloudflare's `PUT /accounts/{account_id}/cfd_tunnel/{tunnel_id}/configurations` API and requires a token with Tunnel Write permission. It never accepts the token as a CLI argument.
...
To pull the latest successful GitHub drill artifact into the local evidence workspace and revalidate it:

```bash
npm run evidence:github-drill -- --json
npm run evidence:github-drill -- --promote --require-current-sha --json
npm run validate:drill-evidence -- --json
```

The first command downloads and validates the latest successful `commercial-drill.yml` artifact under `reports/commercial-evidence/github-drill-artifact/` and writes `reports/commercial-evidence/latest-github-drill-evidence.json`. The `--promote` form copies the artifact's `commercial-evidence/` and `backups/` directories into the normal local evidence paths so `npm run validate:drill-evidence -- --json` can pass without local Docker. Use `--require-current-sha` for release-candidate evidence so an older successful drill cannot be accidentally reused after code changes. The script refuses unsafe repository/run identifiers, keeps generated evidence private, and does not close the production secret, Cloudflare, HR, or storage signoff gaps by itself.
