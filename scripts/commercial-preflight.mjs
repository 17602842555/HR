...
function checkDeploymentArtifacts() {
  ...
  [
    ...
    "OBJECT_STORAGE_ENDPOINT=",
    "OBJECT_STORAGE_SECRET_ACCESS_KEY=",
    "WEB_ORIGIN=https://17602842555.github.io",
    "TRUST_PROXY=0",
    ...
    "CLOUDFLARE_BACKEND_WEB_ORIGIN=https://17602842555.github.io",
    "RUN_DB_SEED=0",
  ...
  [
    ...
    "backend_mode:",
    "RELEASE_BACKEND_MODE: ${{ inputs.backend_mode }}",
    ...
    "umask 077",
    "npm run prepare:release-inputs -- --mode \"$RELEASE_BACKEND_MODE\" --json --output reports/commercial-evidence/signoff-validation/release-inputs.json",
    "npm run validate:production-env -- .env.production --json",
    "npm run validate:cloudflare-backend -- --env .env.production --json",
    "env_args=()",
    "npm run validate:secrets-signoff -- docs/production-secrets-signoff.json \"${env_args[@]}\" --mode \"$RELEASE_BACKEND_MODE\" --json",
  ...
  [
    ...
    "Configure Worker bootstrap admin secret",
    "wrangler secret put CLOUDFLARE_BOOTSTRAP_ADMIN_LOGIN",
    "wrangler secret put CLOUDFLARE_BOOTSTRAP_ADMIN_PASSWORD",
  ...
  [
    ...
    "cloudflare deployment status requires API origin ingress mapping to backend service",
    ...
    "cloudflare deployment status reads tunnel configuration through Cloudflare API without leaking token",
    "cloudflare deployment status API tunnel configuration read fails closed and sanitizes token errors",
  ...
  [
    ...
    "hasWeakSeedPassword",
    "Production seed DEFAULT_ADMIN_PASSWORD must be non-placeholder",
    "isLocalHostname",
    "[::1]",
    "::ffff:7f00:1",
    "Production WEB_ORIGIN must not use example.com",
    "Production OBJECT_STORAGE_ENDPOINT must use https",
    "Production OBJECT_STORAGE_SECRET_ACCESS_KEY must be at least 16 characters",
    "hasPlaceholder",
  ...
  [
    ...
    "runtime treats APP_ENV production as production",
    "production runtime rejects placeholder JWT fragments",
    "production runtime rejects weak seed admin password",
    ...
    "production runtime rejects placeholder S3 object storage credentials",
    "production runtime rejects short S3 object storage secret",
...
function checkCommercialEvidenceAutomation() {
  ...
  [
    ...
    "GitHub release plan keeps production env and drill requirements for tunnel mode",
    ...
    "GitHub release apply triggers native signoff and deploy without Docker drill",
    "GitHub release apply can trigger tunnel drill workflow without waiting"
...
function checkCommercialSmokeCoverage() {
  ...
  [
    ...
    "Unbound approval rule did not fail closed",
    "Smoke approver account was not created",
    "Approval rule did not bind every approver to an account",
    "Approval rule coverage did not include the saved department rule",
    "Approval rule coverage did not mark the saved department rule as fully bound",
...
function checkSystemReadinessImplementation() {
  ...
  [
    ...
    "readiness.signoffDrafts.handoffItemCount",
    "readiness.signoffDrafts.relatedGapIds",
    "readiness.signoffDrafts.requiredActionCount",
  ...
  [
    ...
    "summary.handoffItemCount",
    "summary.relatedGapIds",
