...
test("cloudflare backend validator accepts native worker deployment without tunnel origin", () => {
  const report = validateCloudflareBackendEnv(nativeEnv);

  assert.equal(report.ok, true);
  assert.deepEqual(report.errors, []);
  assert.equal(report.summary.apiOriginConfigured, false);
  assert.equal(report.summary.deploymentUrlConfigured, true);
  assert.equal(report.summary.mode, "native-worker");
  assert.equal(report.summary.tunnelTokenConfigured, false);
});

test("cloudflare backend validator accepts explicit tunnel origin deployment origin and web origin", () => {
  const report = validateCloudflareBackendEnv(validEnv, { mode: "tunnel" });

  assert.equal(report.ok, true);
  assert.deepEqual(report.errors, []);
  assert.equal(report.summary.apiOriginConfigured, true);
  assert.equal(report.summary.deploymentUrlConfigured, true);
  assert.equal(report.summary.mode, "tunnel");
...
test("cloudflare backend validator reads dotenv files", () => {
  ...
  try {
    writeFileSync(envPath, Object.entries(nativeEnv).map(([key, value]) => `${key}="${value}"`).join("\n"));
...
test("cloudflare backend validator CLI parser supports env and json flags", () => {
  assert.deepEqual(parseCloudflareBackendArgs(["--env", ".env.production.cloudflare", "--mode", "tunnel", "--json"]), {
